...
const AddCreater = ({ memberModalVisible, setMemberModalVisible, users, res }: AddCreaterProps) => {
  const [errorMessage, setErrorMessage] = useState("");

  const [creaters, setInvites] = useState(res?.creaters || []);
  ...
  const update = async() => {
    const res1 = await dispatch(updatemeeting({_id:res._id,title:res.title, creaters:invite_user, meeting: res.meeting, create_user:res.create_user, invite_users:res.invite_users, period:res.period}))
                ...
                >
                  <option value={""} disabled></option>
                  {
                    users?.map((item:any,index:number)=>(
                      !creaters?.includes(item.username) &&
