...
import { getUser, updatelink } from '../redux/userSlice';
...
const AddTag = ({ modalTag, showMeeting, showTask, showWiki, setModalTag, setShowMeeting, setShowTask, setShowWiki }: AddTagProps) => {
  ...
  const handleAddLink = async(title:string, link:string) => {
    await dispatch(updatelink({link:[...links, {title, link}], Id:user._id}));
          ...
          <div className="flex flex-col px-6.5 pt-2">
            <div className='w-full'>
              ...
              <div className='flex flex-col items-start w-full'>
                ...
                <div onClick={() => setShowAddLinkModal(true)} className='w-full flex items-center py-2 px-3 border rounded-md'>
            ...
            <div className='w-full md:h-10 py-2 mt-2 overflow-y-auto'>
              {
                links?.map((item, index)=>{
                  return <div>
                    {item.title}:<Link to={item.link} key={index} target="_blank" className='underline'>{item.link}</Link>
                  </div>
                })
              }
            </div>            
